...
<?php
/* PAGINADOR */
$slug = '?action=ver_facturas';
$current = (   isset($_GET['page'])   )? $_GET['page'] : 1 ;
	$current = (   $current>0   )? $current : 1 ;
$post_per_page = 50;

		//Total de resultados
		$t_consulta = "SELECT * FROM facturas WHERE UID = ".$_SESSION['ID'];
...
<div id="cuerpo_action">
	<p id="cabecera_action">Inicio > Ver Facturas</p>
	
	<?php
			/* Extraemos los clientes */
			$consulta = "SELECT * FROM facturas WHERE UID = ".$_SESSION['ID']." ORDER BY FID DESC LIMIT ".$apartir_limit.",".$post_per_page;
			...
			?>
				<div id="cliente-id-<?=$row['FID']?>" class="lista_row <?php if($aux%2==1){echo 'par';}else{echo 'inpar';} ?>">
					...
					<span class="cactions">
						...
						<a href="?action=edit_factura&id=<?=$row['FID']?>" title="Editar"><img src="img/edit.png" /></a>
