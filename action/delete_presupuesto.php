...
if(   isset($_GET['id']) && $_GET['id']>0   ) {
	...
	@mysqli_free_result($query);
		...
		if(   isset($_POST['after'])   ) {		
				...
				if(   $_SESSION['admin']==1   ) {
						$update = "DELETE FROM presupuestos WHERE PID=".$pid;
				} else {
						$update = "DELETE FROM presupuestos WHERE UID=".$_SESSION['ID']." AND PID=".$pid;
				}
				mysqli_query($db, $update);

				if(   $_GET['action'] == 'admin_delete_presupuesto'   ) {
		?>
						<script>
							window.location.href = '?action=admin_ver_presupuestos';
						</script>
		<?php
				} else {
		?>
						<script>
							window.location.href = '?action=ver_presupuestos';
						</script>
		<?php
				}
				exit;
...
<div id="cuerpo_action">
	<p id="cabecera_action">Inicio > Eliminar Presupuesto</p>
	
	<form id="form_edit_client" method="POST" action="">
			<h1>Esta seguro de eliminar este presupuesto?</h1>
			<em>
