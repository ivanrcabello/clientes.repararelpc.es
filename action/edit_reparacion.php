...
if(   isset($_POST['seenvio'])   ) {
		...
		if(   $img_validate   ) {
				...
				?>
				<script>
					window.location.href = '?action=ver_reparaciones&last=<?=$aid?>';
				</script>
				<?php
		} else {
...
<div id="cuerpo_action">
	...
	<form id="form_edit_client" method="POST" action="" enctype="multipart/form-data">
		...
		<div class="cliente-solo-admin"  style="<?= $_SESSION['admin']==1 ?   'display: block' : 'display: none' ?>">
			...
			<div class="todo-admin" style="<?= $_SESSION['admin']==1 ?   'display: block' : 'display: none' ?>">
			...
			<!---
			<input type="text" value="<?=$row['dire']?>" id="address" readonly />-->
			<select name="puntos" id="puntos" style="width: 100%;background: white;border-bottom: 5px solid #999;margin-bottom: 5px;padding: 1rem;">
                <?php
		        ...
		        if (mysqli_num_rows($query)>0) {
    				while (   $rowx =  mysqli_fetch_array($query, MYSQLI_ASSOC)   ) {
    				    ?>
    				    <option value="<?php echo $rowx["GID"]; ?>" <?php echo $row['punto'] === $rowx['GID'] ? 'selected=true' : '' ?>><?php echo $rowx["nombre"]." - ".$rowx["direccion"]; ?></option>
    				    <?php
    				}
		        }
                ?>
			...
			<br/>
			<div class="todo-lo-demas-admin" style="<?= $_SESSION['admin']==1 ?   'display: block' : 'display: none' ?>">
			...
			<label class="accesorio"><input type="radio" name="status" value="10" <?php if($row['status']==10){ echo "checked";} ?> /> Sin recepcionar</label>
			...
			<label class="accesorio"><input type="radio" name="status" value="4" <?php if($row['status']==4){ echo "checked";} ?> /> En Garantía</label>
			<label class="accesorio"><input type="radio" name="status" value="11" <?php if($row['status']==11){ echo "checked";} ?> /> Electrónica</label>
			<label class="accesorio"><input type="radio" name="status" value="12" <?php if($row['status']==12){ echo "checked";} ?> /> No reparable</label>
			<label class="accesorio"><input type="radio" name="status" value="13" <?php if($row['status']==13){ echo "checked";} ?> /> Esperando material</label>
			<label class="accesorio"><input type="radio" name="status" value="14" <?php if($row['status']==14){ echo "checked";} ?> /> Presupuestando</label>
			<br/><br/>
