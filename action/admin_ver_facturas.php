...
<?php
/* PAGINADOR */
$slug = '?action=admin_ver_facturas';
$current = (   isset($_GET['page'])   )? $_GET['page'] : 1 ;
	$current = (   $current>0   )? $current : 1 ;
$post_per_page = 50;

		//Total de resultados
		$t_consulta = "SELECT * FROM facturas";
...
<div id="cuerpo_action">
	...
	<?php
			/* Extraemos las facturas */
			$consulta = "SELECT * FROM facturas ORDER BY FID DESC LIMIT ".$apartir_limit.",".$post_per_page;
			...
			?>
				<div id="cliente-id-<?=$row['FID']?>" class="lista_row <?php if($aux%2==1){echo 'par';}else{echo 'inpar';} ?>">
					...
					<span class="cactions">
						...
						<a href="?action=admin_delete_factura&id=<?=$row['FID']?>" title="Eliminar"><img src="img/trash.png" /></a>
					</span>
				</div>
			<?php
						}
