...
if(   isset($_GET['id']) && $_GET['id']>0   ) {
	...
	@mysqli_free_result($query);
		...
		if(   isset($_POST['after'])   ) {		
				...
				if(   $_SESSION['admin']==1   ) {
						$update = "DELETE FROM puntos WHERE GID=".$gid;
				} else {
						$update = "DELETE FROM puntos WHERE UID=".$_SESSION['ID']." AND GID=".$gid;
				}
				mysqli_query($db, $update);

				if(   $_GET['action'] == 'admin_delete_punto'   ) {
		?>
						<script>
							window.location.href = '?action=admin_ver_puntos';
						</script>
		<?php
				} else {
		?>
						<script>
							window.location.href = '?action=ver_puntos';
						</script>
		<?php
				}
				exit;
...
<div id="cuerpo_action">
	<p id="cabecera_action">Inicio > Punto Gasto</p>
	
	<form id="form_edit_client" method="POST" action="">
			<h1>Esta seguro de eliminar este punto?</h1>
			<em>
